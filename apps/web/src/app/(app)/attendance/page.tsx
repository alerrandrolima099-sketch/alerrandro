...
// Atendimentos (seção 38): esta é a antiga página "Conversas" - inbox real
// de clientes, com envio/recebimento ao vivo via WebSocket e retry de
// mensagem falhada. Só mudou de rota/nome na navegação; o comportamento é
// 100% o mesmo que já estava validado em produção. A rota "/conversations"
// agora é o monitor de interações de aquecimento (ver aquele arquivo).
    ...
    <div className="h-[calc(100vh-4rem)] -m-6 md:-m-8 flex bg-background">
      <div className="w-full md:w-80 border-r border-border overflow-y-auto flex flex-col">
        <div className="p-4 border-b border-border flex items-center justify-between">
          <h1 className="font-semibold">Atendimentos</h1>
          <span
        ...
        {conversations.length === 0 && <p className="p-4 text-sm text-muted">Nenhum atendimento ainda.</p>}
      </div>
      ...
      {selected ? (
        ...
        <div className="flex-1 flex items-center justify-center text-muted text-sm">Selecione um atendimento</div>
      )}
    </div>
  );
}
